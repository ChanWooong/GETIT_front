...
// 백엔드 College Enum과 1:1 매칭 (readOnly 시 표시명 조회용으로 export)
...
const CollegeSelect = ({ onChange, value, readOnly }) => {
  const collegeList = COLLEGE_LIST;
  const displayName = value ? (collegeList.find((c) => c.id === value)?.name ?? value) : '';
  ...
  if (readOnly) {
    return (
      <div className="space-y-2 text-left">
        ...
        <div className="w-full bg-black/30 border border-white/10 rounded-xl py-3 px-4 text-white cursor-default">
          {displayName || '—'}
        </div>
      </div>
    );
